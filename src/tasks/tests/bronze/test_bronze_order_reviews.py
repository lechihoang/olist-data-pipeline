...
# --- SCHEMA DEFINITION ---
class BronzeOrderReviewsSchema(DataFrameModel):
    """
    Pandera schema for bronze.order_reviews_raw table.
...
def run_tests(spark, table_name: str) -> dict:
    ...
    df_validated = BronzeOrderReviewsSchema.validate(check_obj=df)
    ...
    result = {
        ...
        "errors": errors,
        "row_count": df.count(),
        "schema_name": "BronzeOrderReviewsSchema"
    }
...
print(f"Using Pandera schema: BronzeOrderReviewsSchema")

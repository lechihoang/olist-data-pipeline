...
# --- SCHEMA DEFINITION ---
class BronzeCustomersSchema(DataFrameModel):
    """
    Pandera schema for bronze.customers_raw table.
...
def run_tests(spark, table_name: str) -> dict:
    ...
    df_validated = BronzeCustomersSchema.validate(check_obj=df)
    ...
    result = {
        ...
        "errors": errors,
        "row_count": df.count(),
        "schema_name": "BronzeCustomersSchema"
    }
...
print(f"Using Pandera schema: BronzeCustomersSchema")

...
def run_tests(spark: SparkSession, table_name: str):
    ...
    # Create ephemeral GX context (no file config needed)
    context = gx.get_context(mode="ephemeral")
    
    # Create Spark datasource
    datasource = context.sources.add_or_update_spark("spark_ds")
    data_asset = datasource.add_dataframe_asset(name="customers_raw")
    batch_request = data_asset.build_batch_request(dataframe=df)
    
    # Create expectation suite
    suite = context.add_or_update_expectation_suite("bronze_customers_suite")

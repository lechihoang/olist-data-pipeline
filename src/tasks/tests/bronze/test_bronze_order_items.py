...
# Test Bronze Order Items - Validates data quality for order_items_raw table
...
# --- SCHEMA DEFINITION ---
class BronzeOrderItemsSchema(DataFrameModel):
    """
    Pandera schema for bronze.order_items_raw table.
...
def run_tests(spark, table_name: str) -> dict:
    ...
    df_validated = BronzeOrderItemsSchema.validate(check_obj=df)
    ...
    result = {
        ...
        "errors": errors,
        "row_count": df.count(),
        "schema_name": "BronzeOrderItemsSchema"
    }
...
print(f"Using Pandera schema: BronzeOrderItemsSchema")

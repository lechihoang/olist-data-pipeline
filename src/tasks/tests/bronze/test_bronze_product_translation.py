...
# Requirements: 1.9 - Verify product_category_name NOT NULL

from pyspark.sql import SparkSession
from pyspark.sql.functions import col

def run_tests(spark: SparkSession, table_name: str) -> list:
    """Run data quality tests for bronze product_category_name_translation table"""
    results = []
    ...
    # Test 2: product_category_name NOT NULL
    null_category = df.filter(col("product_category_name").isNull()).count()
    results.append({
        "test_name": "product_category_name_not_null",
        "passed": null_category == 0,
        "details": f"NULL product_category_name count: {null_category}",
        "row_count": row_count
    })

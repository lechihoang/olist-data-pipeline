...
# Test Bronze Customer - Validates data quality for customer_raw table
...
# --- SCHEMA DEFINITION ---
class BronzeCustomerSchema(DataFrameModel):
    """
    Pandera schema for bronze.customer_raw table.
...
def run_tests(spark, table_name: str) -> dict:
    ...
    df_validated = BronzeCustomerSchema.validate(check_obj=df)
    ...
    result = {
        ...
        "errors": errors,
        "row_count": df.count(),
        "schema_name": "BronzeCustomerSchema"
    }
...
logger.info(f"Using Pandera schema: BronzeCustomerSchema")

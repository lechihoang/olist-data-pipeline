...
# Test Bronze Order Item - Validates data quality for order_item_raw table
...
# --- SCHEMA DEFINITION ---
class BronzeOrderItemSchema(DataFrameModel):
    """
    Pandera schema for bronze.order_item_raw table.
    ...
    shipping_limit_date: T.StringType() = Field(nullable=True)
    price: T.DoubleType() = Field(nullable=True)
    freight_value: T.DoubleType() = Field(nullable=True)
...
def run_tests(spark, table_name: str) -> dict:
    ...
    df_validated = BronzeOrderItemSchema.validate(check_obj=df)
    ...
    result = {
        ...
        "errors": errors,
        "row_count": df.count(),
        "schema_name": "BronzeOrderItemSchema"
    }
...
logger.info(f"Using Pandera schema: BronzeOrderItemSchema")
